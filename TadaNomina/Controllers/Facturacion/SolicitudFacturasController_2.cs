...
namespace TadaNomina.Controllers.Facturacion
{
    public class SolicitudFacturasController : BaseController
    {
        ...
        public JsonResult NuevaSolicitud(string _PeriodoNomina, string _Esquema, string _Importe, string _Honorario, string _Subtotal, string _IVA, string _Total)
        {
            ...
            try
            {
                if (int.Parse(_Subtotal) > 0 && int.Parse(_IVA) > 0 && int.Parse(_Total) > 0)
                {
                    var csf = new ClassSolicitudFacturas();
                    csf.newSolicitud((int)Session["sIdCliente"], Session["sCliente"].ToString(), (int)Session["sIdUnidadNegocio"], Session["sNomina"].ToString(), _PeriodoNomina, _Esquema, int.Parse(_Importe), int.Parse(_Honorario), int.Parse(_Subtotal), int.Parse(_IVA), int.Parse(_Total), (int)Session["sIdUsuario"]);
        ...
        public ActionResult Solicitud(ModelSolicitudFacturas msf)
        {
            try
            {
                ...
                if (ModelState.IsValid && msf.validacion == 1)
                {
                    ...
                    string ruta = @"C:\\ArchivosFactura\" + carpeta;
        ...
        public ActionResult SolicitudPendienteTesoreria(ModelSolicitudFacturas msf)
        {
            ...
            string Correos = n.ExtraeCorreosTesoreria("FACTURACION", (int)Session["sIdCliente"], (int)Session["sIdUnidadNegocio"],null);

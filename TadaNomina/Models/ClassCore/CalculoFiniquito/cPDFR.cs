...
namespace TadaNomina.Models.ClassCore.CalculoFiniquito
{
    public class cPDFR
    {
        ...
        /// <returns></returns>
        public byte[] getPDFReal(int IdPeriodonomina, int IdEmpleado)
        {
            ...
            //////////////////////////////////////////////////////////////////////////////////////
            string _TipoCalculo = "CÁLCULO REAL";
            ...
            PdfPCell SMIMSS = new PdfPCell(new Phrase(((decimal)NominaTrab.SueldoDiarioReal * 30).ToString("C2"), fontCuerpo));
            ...
            PdfPCell salarioDiario = new PdfPCell(new Phrase((((decimal)NominaTrab.SueldoDiarioReal)).ToString("C2"), fontCuerpo));
            ...
            var sueldo = NominaTrab.Apoyo + NominaTrab.SueldoPagado;
            if (sueldo != 0)
            {
                ...
                PdfPCell sueldoTotales = new PdfPCell(new Phrase(string.Format("{0:C2}", sueldo), fontCuerpo));
            ...
            foreach (var item in incidencias.Where(x => x.TipoConcepto == "ER" && x.IdEstatus == 1).ToList())
            {
                ...
                decimal montoEsq = 0;
                decimal montoTrad = 0;
                try { montoEsq = (decimal)item.MontoEsquema; } catch { }
                try { montoTrad = (decimal)item.Monto; } catch { }
                try { montoReal = montoEsq + montoTrad; } catch { }
                ...
                if (montoReal != 0)
                {
                    ...
                    PdfPCell ConceptoTotales = new PdfPCell(new Phrase(string.Format("{0:C2}", item.MontoEsquema + item.Monto), fontCuerpo));
            ...
            PdfPCell TOTALPercepciones2 = new PdfPCell(new Phrase(string.Format("{0:C2}", NominaTrab.ERS + NominaTrab.ER), fontCuerponegrita));
            ...
            if ((NominaTrab.ImpuestoRetener + NominaTrab.ISRLiquidacion) != 0)
            {
                ...
                PdfPCell ISRTotal = new PdfPCell(new Phrase(String.Format("{0:C2}", (NominaTrab.ISR + NominaTrab.ISRLiquidacion)), fontCuerpo));
                ISRTotal.Colspan = 20;
            ...
            {
                if (item.MontoEsquema != 0 || item.Monto != 0)
                {
                    PdfPCell ClaveConcepto = new PdfPCell(new Phrase(item.ClaveConcepto, fontCuerpo));
                    ClaveConcepto.Colspan = 10;
                    ClaveConcepto.BorderWidthBottom = 0;
            ...
            PdfPCell TOTALDeducciones2 = new PdfPCell(new Phrase(string.Format("{0:C2}", NominaTrab.DDS + NominaTrab.DD), fontCuerponegrita));
            ...
            PdfPCell TOTALPercepciones23 = new PdfPCell(new Phrase(((float)NominaTrab.ERS + (float)NominaTrab.ER).ToString("C2"), fontCuerponegrita));
            ...
            PdfPCell TOTALDeducciones23 = new PdfPCell(new Phrase(((float)NominaTrab.DDS + (float)NominaTrab.DD).ToString("C2"), fontCuerponegrita));
            ...
            decimal Netos = ((decimal)NominaTrab.ERS + (decimal)NominaTrab.ER) - ((decimal)NominaTrab.DDS + (decimal)NominaTrab.DD);
